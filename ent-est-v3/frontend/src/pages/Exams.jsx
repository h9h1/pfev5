import { FileText, Plus, Upload, Download, Trash2, X, Check, Loader } from 'lucide-react'
...
export default function Exams({ user }) {
  ...
  const openExam=async(exam)=>{
    setSelected(exam)
    if(isTeacher){try{const r=await examsAPI.listSubmissions(exam.id);setSubs(r.data)}catch{setSubs([])}}
  }
        ...
        {exams.map(e=>(
          <div key={e.id} style={{...T.card,borderTop:`3px solid ${TC[e.exam_type]||'#2d8a4e'}`,position:'relative'}}
            ...
            <div style={{display:'flex',gap:8}}>
              ...
              {isTeacher&&<span style={T.meta}>{e.submission_count||0} copies</span>}
            </div>
          </div>
        ))}
      </div>
                    ...
                    {subs.map(s=>(
                      <div key={s.id} style={{...T.fileItem,flexWrap:'wrap',gap:8}}>
                        <div style={{flex:1}}>
                          ...
                          {s.grade!=null&&<div style={{fontSize:12,color:'#10b981',fontWeight:600}}>✓ {s.grade}/20</div>}
                        </div>
...
const T={
  ...
  btn:{background:'linear-gradient(135deg,#2d8a4e,#1a5c32)',border:'none',borderRadius:9,color:'#fff',padding:'9px 16px',fontSize:13,fontWeight:600,cursor:'pointer',whiteSpace:'nowrap'},
  ...
  dlBtn:{background:'rgba(45,138,78,.12)',border:'1px solid rgba(45,138,78,.3)',borderRadius:7,color:'var(--accent2)',padding:'7px 12px',fontSize:12,cursor:'pointer',fontWeight:500,whiteSpace:'nowrap'},
  ...
  secTitle:{fontSize:11,fontWeight:700,color:'var(--text3)',textTransform:'uppercase',letterSpacing:1,marginBottom:12,paddingBottom:8,borderBottom:'1px solid var(--border)'},
